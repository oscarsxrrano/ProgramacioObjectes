 ...
 * @return outputValue Output value
 */
fun readColor(pMessageIn: String
              , pMessageErrorDT: String
              , pMessageErrorColor: String
): String{

    var outputValue: String = ""
    var correctDataType: Boolean = false
    val listaColors = listOf("blanc", "blau", "vermell", "groc", "verd")
    var outputValueLower = ""
    do{
        println(pMessageIn)
        correctDataType = scan.hasNext()
        ...
        if (!correctDataType){
            ...
            outputValueLower = outputValue.lowercase(Locale.getDefault())
        }

        if (outputValueLower !in listaColors) println(YELLOW_BOLD +"ERROR: " + pMessageErrorColor + RESET)
    }while(!correctDataType || outputValueLower !in listaColors)

    return outputValueLower
}
